...
class hhm: 
    ...
    def set_dynamic_pars(self, omega, n_p=1, n_t=300, return_domain=False):
        ...
        #tvec = np.linspace(0,  num_periods* 2*np.pi/omega,  num_periods*Nt+1)
    ...
    def set_Htpars(self, El, phase, delta_n=0.5, Ej0=0.5, Ec0=1.0,  
        ...
        self.Hsc_J = Hsc_Josephson(self.system_size,self.Ej,self.El,self.phase, self.Mcut, self.pbc)
    ...
    def hamiltonian(self, t, args={}):
        ...
        Parameters:
            t (float): time
            args: dictionary with Hamiltonian parameters
                'L': number of sites
                'delta_n':amplitude of the driving of the induced charge
                'Ec': charging energy of each island
                'Mcut': truncation of the local Hilbert space
                'omega': frequency of the driving.
        ...
        Ec, Ej = self.Ec, self.Ej
        Mcut = self.Mcut 
        
        Ec = expand_variable( Ec, L )
        ng = self.ng_t( omega * t ) 
        ...
        for j in range(L):
            ...
            op_list[j] = Ec[j] * ( num(Mcut) - ng[j] )**2
            H += tensor(op_list)
            op_list[j] = qeye(Mcut)
    ...
    def initial_state(self):
        ...
        ng = 0.5 + self.ng_t(0)
        Hsc = Hsc_chain_static(L, ng, self.Ec, self.Ej, 0, 0, Mcut=Mcut, PBC=PBC) 
    ...
    def get_ops(self):
        ...
        L = self.system_size 
        Mcut = self.Mcut 
        Ej, El = self.Ej, self.El
        phase = self.phase
        
        tot_current, current_density = get_bond_current(L, Mcut, Ej, El, 
                                                        phase, PBC=self.pbc)
        tot_num = total_number(L, Mcut, density=True)
...
def total_number(L, Mcut, density=False):
    '''
...
def get_bond_current(L,Mcut, Ej, El, phlead, PBC=False):
    '''
...
def Hsc_Josephson(L,Ej,El,phlead, Mcut=2, PBC=False):
    '''
